...
public class WeightedGraph<E> {
	...
	public int dijkstrasAlgorithm(E originVertex, E targetVertex) {
		// Initialize priority queue and skip list
		PriorityQueue<GraphNode<E>> vertexQueue = new PriorityQueue<>();
		ConcurrentSkipListMap<E, Integer> distances = new ConcurrentSkipListMap<>();

		// Set all distances to not found (Infinity) and set prev's to -1
		for (int i = 0; i < vertices; i++) {
			distances.put(adjList[i].get(0).getData(), Integer.MAX_VALUE);
			vertexQueue.add(adjList[i].get(0));
		}

		// Search for shortest path while queue is not empty
		distances.put(originVertex, 0);
		while (!vertexQueue.isEmpty()) {
			// Get vertex with smallest distance
			GraphNode<E> currentVertex = vertexQueue.poll();
			
			// Check if at target
			if (currentVertex.getData().equals(targetVertex)) {
				return distances.get(currentVertex.getData());
			}
			
			// For each neighbor of the currentVertex
			int indexOfCurrentVertex = getIndexOfElement(currentVertex.getData());
			for (GraphNode<E> neighbor : adjList[indexOfCurrentVertex]) {
				// Continue if the neighbor is the currentVertex
				if (currentVertex.equals(neighbor)) {
					continue;
				}
				
				// Calculate new distance
				int altDist = distances.get(currentVertex.getData()) + neighbor.getWeight();
				
				// Update distance if shorter path
